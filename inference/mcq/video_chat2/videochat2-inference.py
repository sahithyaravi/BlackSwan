...
with open('../../../results/VAR_Data_videochat2.json', 'r') as f:
...
task1_prompt = (
        "Describe what could happen next, by explaining the sequence of actions leading to the outcome."
    )

task2_prompt = (
        "What happened in the missing frames (in black) of the video?"
    )
...
for item in tqdm(VAR_Data):

    if "task3_response" in item:
        continue

    for task in tasks:
        if task == "task1":
            question = task1_prompt
            video_path = os.path.join(base_path, f"{item['set_id']}", item['preevent'])
    
        elif task == "task2":
            question = task2_prompt
            video_path = os.path.join(base_path, f"{item['set_id']}_merged", f"{item['index']}_D_merged.mp4")
    
        else:
            question = task3_prompt
            video_path = os.path.join(base_path, f"{item['set_id']}_merged", f"{item['index']}_E_merged.mp4")
        ...
        img_list = []
        with torch.no_grad():
            image_emb, _, _ = model.encode_img(video, "Carefully watch the video and pay attention to the cause and sequence of events, the detail and movement of objects, and the action and pose of persons. Based on your observations, select the best option that accurately addresses the question.\n")
        ...
        llm_message = answer(conv=chat, model=model, do_sample=False, img_list=img_list, max_new_tokens=512, print_res=False)[0]
        out = llm_message.strip()
        #print(out)

        print(task, out)

        item[f'{task}_response'] = out

        with open('../../../results/VAR_Data_videochat2.json', 'w') as f:
