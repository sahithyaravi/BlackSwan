...
def extract_letter(text):
    # Check if "yes" or "no" is present in the text (case-insensitive)
...
def do_inference(mcq_list, base_path, model, processor, tokenizer, conv_mode, output_file):
    ...
    for q in tqdm(mcq_list, desc="Processing MCQs"):
        ...
        hypo = q["exp"]
        label = q["def_label"].lower()  # Lowercase the label for comparison

        # Set the video path and question based on the task
        if curr_task == 1:
        ...
        # Perform inference
        paths = [video_path]  # Only one video path is required now
        output = inference(paths, [question], model, processor, tokenizer, conv_mode)
        q["predicted"] = output.lower()  # Ensure the model's output is lowercase

        print("Model Answer:", output)

        # Check if the output is correct
        if output and output.lower() in ["yes", "no"]:
            is_correct = (output.lower() == "yes" and label == "yes") or (output.lower() == "no" and label == "no")
            if is_correct:
                task_stats[curr_task]["correct"] += 1
            else:
                task_stats[curr_task]["failed"] += 1
        else:
            task_stats[curr_task]["failed"] += 1

        task_stats[curr_task]["total"] += 1

        # Calculate and print running accuracy for the current task type
        correct = task_stats[curr_task]["correct"]
        total = task_stats[curr_task]["total"]
        accuracy = correct / total if total > 0 else 0
        print(f"Running Accuracy for Task {curr_task}: {accuracy:.4f}")
