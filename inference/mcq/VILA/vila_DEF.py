    ...
    def process_questions(self, args):
        ...
        for item in tqdm(self.dataset, desc="Processing Questions"):
            ...
            try:
                ...
                # Define the question text and label
                hypo = item["exp"]
                question_text = f"Hypothesis: {hypo}\nGiven the frames, does this hypothesis hold? Answer yes or no."
                label = item["def_label"].lower()  # Make label lowercase for comparison
                task_type = item["def_task"]

                # Use appropriate frames based on the task type
                if task_type == 2:
                    out = self.generate_and_save_answer_task2(
                        question_text, 
                        ["yes", "no"], 
                        label, 
                        preevent_frames[:5], 
                    ...
                    out = self.generate_and_save_answer(
                        question_text, 
                        ["yes", "no"], 
                        label, 
                        preevent_frames, 
                ...
                item["predicted"] = out.lower()  # Ensure model output is lowercase

                # Check if the output is correct
                if out and out.lower() in ["yes", "no"]:
                    is_correct = (out.lower() == label)
                    if is_correct:
