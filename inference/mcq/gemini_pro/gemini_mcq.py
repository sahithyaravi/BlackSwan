...
def gemini_call(video_path: str, question: str, save_file_name: str) -> str:
    ...
    with open(save_file_name, 'a') as f:
        try:
            ...
            return out["Answer"] if "Answer" in out else out
        except Exception as e:
            print(f"Error processing file: {e}")
            return ""

def do_inference(mcq_list: list, base_path: str, save_file_name: str):
    """Perform inference for multiple-choice QA across both task types."""
    ...
    for q in tqdm(mcq_list, desc="Processing MCQs"):
        ...
        if task_type == 1:
            question = (
                ...
                f"(A) {A}\n(B) {B}\n(C) {C}.\n Enclose the output as a json with key 'Answer' and value as your choice."
            )
            video_suffix = "D"
        elif task_type == 2:
            question = (
        ...
        correct_option = ["A", "B", "C"][q["mcq_label"]]
